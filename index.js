...
const getConstructor = (o) => {
  switch (o.t) {
    ...
    case Deep:
      return deep;
    default:
      throw "getConstructor called on incompatible type";
  }
};
...
const concatWithMiddle = (o1, mid, o2) => {
  ...
  if (o1.t === Single)
    return prependMany(o2, concatIters(revArrIter(mid), [o1.content]));
  if (o2.t === Empty) return appendMany(o1, mid);
  if (o2.t === Single) return appendMany(o1, concatIters(mid, [o2.content]));
  if (!(o1.t === Deep && o2.t === Deep))
    throw "concatWithMiddle called with unexpected tree types!";
  const l1 = o1.left;
  const m1 = o1.middle;
  const r1 = o1.right;
  const l2 = o2.left;
...
const measure = (o, m) => {
  ...
  switch (o.t) {
    ...
    case Deep:
      result = m.sum(
        measure(o.left, m),
        m.sum(measure(o.middle, m), measure(o.right, m))
      );
      break;
    case Branch2:
    ...
    case Four:
      result = m.sum(
        measure(o.a, m),
        m.sum(measure(o.b, m), m.sum(measure(o.c, m), measure(o.d, m)))
      );
      break;
    default:
...
const deepWithRebalance = (leftArr, middle, rightArr) => {
  ...
  if (leftArr.length > 4 || rightArr.length > 4)
    throw "Affixes cannot be longer than 4 elements";
...
const splitWithMid = (o, m, pred, offset) => {
  switch (o.t) {
    case Empty:
      return null;
    case Single:
      return pred(m.sum(offset, measure(o, m)))
        ? [empty(), o.content, empty()]
        : null;
    case Deep: {
      ...
      if (pred(offset2)) {
        const [before, [x, ...after]] = splitList(
          affixToList(o.left),
          m,
          pred,
          offset
        ...
        return [
          chunkToTree(before),
          x,
          deepWithRebalance(after, o.middle, affixToList(o.right)),
        ];
      }
      ...
      if (pred(offset3)) {
        ...
        const [beforeNode, [x, ...afterNode]] = splitList(
          nodeToList(node),
          m,
          pred,
          m.sum(offset2, measure(before, m))
        );
        return [
          deepWithRebalance(affixToList(o.left), before, beforeNode),
          x,
          deepWithRebalance(afterNode, after, affixToList(o.right)),
        ];
      }
      ...
      return [
        deepWithRebalance(affixToList(o.left), o.middle, before),
        x,
        chunkToTree(after),
      ];
    }
...
const getParts = (o) => {
  switch (o.t) {
    ...
    case Single:
      return [o.content];
    case Deep:
      return [o.left, o.middle, o.right];
    case Branch2:
      return [o.a, o.b];
    case Branch3:
      return [o.a, o.b, o.c];
    ...
    default:
      throw "getParts called on incompatible type!";
  }
};

const callFirstTruePart = (o, m, pred, offset, f) => {
  let nextOffset;
  let parts = getParts(o);
  for (let i = 0; i < parts.length; i++) {
    let part = parts[i];
    nextOffset = m.sum(offset, measure(part, m));
    if (pred(nextOffset)) return f(part, i, offset);
    offset = nextOffset;
  }
  return null;
};
...
const getFirstTrue = (o, m, pred, offset) => {
  if (isData(o)) return o;
  if (o.t == Empty) return null;
  return callFirstTruePart(o, m, pred, offset, (part, i, offset) =>
    getFirstTrue(part, m, pred, offset)
  );
};

const updateHead = (o, f) => {
  switch (o.t) {
    case Empty:
      return o;
    case Single:
      return single(f(o.content));
    case Deep:
      const newHead = f(o.left.a);
      const left = o.left;
      let newLeft;
      switch (left.t) {
        case One:
          ...
          break;
        case Two:
          newLeft = two(newHead, left.b);
          break;
        case Three:
          newLeft = three(newHead, left.b, left.c);
          break;
        case Four:
          newLeft = four(newHead, left.b, left.c, left.d);
          break;
        default:
          throw "Error!, this shouldn't happen.";
      }
      return deep(newLeft, o.middle, o.right);
    default:
      throw "updateHead called on incompatible type!";
  }
};

const updateLast = (o, f) => {
  switch (o.t) {
    case Empty:
      return o;
    case Single:
    ...
    case Deep:
      ...
      switch (right.t) {
        case One:
          newRight = one(f(right.a));
          break;
        case Two:
          newRight = two(right.a, f(right.b));
          break;
        case Three:
          newRight = three(right.a, right.b, f(right.c));
          break;
        case Four:
          newRight = four(right.a, right.b, right.c, f(right.d));
          break;
        default:
          throw "Error!, this shouldn't happen.";
      }
      return deep(o.left, o.middle, newRight);
    default:
      throw "updateHead called on incompatible type!";
  }
};

const updatePart = (o, partIndex, newPart) => {
  const parts = getParts(o);
  parts[partIndex] = newPart;
  return getConstructor(o)(...parts);
};

const updateFirstTrue = (o, m, pred, offset, f) => {
  if (isData(o)) return f(o);
  if (o.t == Empty) return o;
  return callFirstTruePart(o, m, pred, offset, (part, i, offset) =>
    updatePart(o, i, updateFirstTrue(part, m, pred, offset, f))
  );
};

const treeMap = (o, f) => {
  if (isData(o)) return f(o);
  return getConstructor(o)(...getParts(o).map((x) => treeMap(x, f)));
};

const mapTrue = (o, f, m, pred) => {
  const _mapTrueParts = (parts, left, right) => {
    //Copy the array to be safe
    parts = [...parts];
    const rights = [...parts]
      .reverse()
      .reduce((s, x) => [m.sum(measure(x, m), s[0]), ...s], [right]);
    for (let i = 0; i < parts.length; i++) {
      let newLeft = m.sum(left, measure(parts[i], m));
      if (pred(newLeft, m.sum(newLeft, rights[i + 1]))) {
        parts[i] = _mapTrue(parts[i], left, rights[i + 1]);
        newLeft = m.sum(left, measure(parts[i], m));
      }
      left = newLeft;
    }
    return parts;
  };
  const _mapTrue = (x, left, right) => {
    if (isData(x)) {
      newLeft = m.sum(left, measure(x, m));
      total = m.sum(newLeft, right);
      return f(x, newLeft, total);
    }
    return getConstructor(x)(..._mapTrueParts(getParts(x), left, right));
  };
  return _mapTrue(o, m.zero(), m.zero());
};
...
class FingerTree {
  ...
  split(pred, offset = NoOffset) {
    ...
    if (!pred(this.m.sum(offset, measure(this.t, this.m))))
      return [this, new FingerTree(this.m)];
    const [before, x, after] = splitWithMid(this.t, this.m, pred, offset);
    return [
      new FingerTree(this.m, before),
      new FingerTree(this.m, prepend(after, x)),
    ];
  }
  ...
  firstMatch(pred, offset = NoOffset) {
    if (offset == NoOffset) offset = this.m.zero();
    return getFirstTrue(this.t, this.m, pred, offset);
  }

  updateFirstMatch(updateFunction, pred, offset = NoOffset) {
    if (offset == NoOffset) offset = this.m.zero();
    return new FingerTree(
      this.m,
      updateFirstTrue(this.t, this.m, pred, offset, updateFunction)
    );
  }

  updateHead(updateFunction) {
    return new FingerTree(this.m, updateHead(this.t, updateFunction));
  }

  updateLast(updateFunction) {
    return new FingerTree(this.m, updateLast(this.t, updateFunction));
  }

  concat(fingerTree2) {
    if (this.m !== fingerTree2.m)
      throw "Can't concatenate finger trees with different measures!";
    return new FingerTree(this.m, concat(this.t, fingerTree2.t));
  }

  map(mapFunction, newMeasure = NoMeasure) {
    if (newMeasure == NoMeasure) newMeasure = this.m;
    return new FingerTree(newMeasure, treeMap(this.t, mapFunction));
  }

  mapTrue(mapFunction, predicate) {
    return new FingerTree(
      this.m,
      mapTrue(this.t, mapFunction, this.m, predicate)
    );
  }
